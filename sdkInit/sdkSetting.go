...
func createChannel(signIDs []msp.SigningIdentity, info *SdkEnvInfo) error {
	...
	if _, err := chMgmtClient.SaveChannel(req, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer0.example.com")); err != nil {
	...
	for i, org := range info.Orgs {
		...
		if _, err = org.OrgResMgmt.SaveChannel(req, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer0.example.com")); err != nil {
